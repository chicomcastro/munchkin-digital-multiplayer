...
export default function App() {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="card-shell p-6 text-center">
          <div className="text-amber-400 font-bold mb-2">{reconnecting ? 'Reconnecting…' : 'Connecting…'}</div>
          <div className="text-sm opacity-60">{session.roomCode}</div>
          <button className="btn mt-4" onClick={leave}>Leave</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <>
      {screen}
      {errorMsg && (
        <div className="fixed bottom-4 left-1/2 -translate-x-1/2 bg-red-700/90 text-white px-4 py-2 rounded-xl shadow-lg z-50">
          {errorMsg}
        </div>
      )}
      {!connected && <ConnectingBanner />}
      <button onClick={leave} className="fixed top-2 right-2 text-xs opacity-50 underline z-50">leave</button>
    </>
  );
}
